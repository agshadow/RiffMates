# RiffMates/content/models.property
...
class SeekingAd(models.Model):
    ...
    def clean(self):
        if self.seeking == MusicianBandChoice.MUSICIAN:
            # Validate seeking musican case
            if self.band is None:
                raise ValidationError("Band field required when seeking a musican")
            ...
            if self.band is not None:
                raise ValidationError(
                    "Band field shoudl be empty for a musician seeking a band"
                )
